...
//    private static final String STAGING_BUCKET_NAME = String.format("eel-staging-bucket-%s", RandomStringUtils.randomAlphanumeric(8));
    ...
    // todo:  can't we just deserialize the object with gson?
    private static final Function<Map<String, AttributeValue>, Flow> DYNAMO_DB_ITEM_MAPPER = (item) -> {
        return gson.fromJson(item.get("object").s(), Flow.class);

//        return new Flow(
//                UUID.fromString(item.get("object").m().get("id").s()),
//                Integer.parseInt(item.get("object").m().get("version").n()),
////            item.get("author").s(),
...
//                item.get("object").m().get("isFinalized").bool()
//        );
    };
